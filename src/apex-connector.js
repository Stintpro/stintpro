...
window.ApexConnector = {
      ...
      if(lapM){
        ...
        if(ms>=20000&&ms<300000){
          ...
          if(!k._lapInvalid){
            ...
            // Solo usar |*| para tiempos si el circuito no tiene columna llp.
            // Con llp, las celdas son la fuente de verdad (llegan antes que |*|).
            if(!this._colMap.llp){
              const t=parseFloat((ms/1000).toFixed(3));
              k.lastLap=t;
              if(k._nextLapDirty){k._nextLapDirty=false;}
              else{k.lapHistory.push(t);if(k.lapHistory.length>1500)k.lapHistory.shift();}
  ...
  _applyCell(k, col, type, val){
    ...
    if(isStateCol||isStateCode){
      ...
      else if(type==='so'){k.pit=true; k.pitState='out'; k.pitS=0; k._pitTimerActive=false; k._pitInTime=null; k._nextLapDirty=true;}
    ...
    if(dtype==='llp'){
      ...
      if(t&&t>=20&&t<300){
        this._lastLapTime=Date.now();
        k.lastLap=t;
        if(k._nextLapDirty){k._nextLapDirty=false; k._inLapPending=false;}
        else{k.lapHistory.push(t);if(k.lapHistory.length>1500)k.lapHistory.shift(); k._inLapPending=true;}
        if(!k.bestLap||t<k.bestLap)k.bestLap=t;
      }
      return;
    }
