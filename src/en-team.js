  ...
  overlay.innerHTML=`
    <div style="background:#1a1b22;border:0.5px solid #2a2b2e;border-radius:12px;padding:24px;max-width:320px;width:90%;text-align:center">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button onclick="_enConfirmDeleteStint(${idx})" style="flex:1;padding:8px;border-radius:6px;border:0.5px solid #ef4444;background:#ef444418;color:#ef4444;font-size:11.5px;cursor:pointer;font-family:sans-serif">Borrar</button>
      </div>
    </div>`;
...
function _enStintDetail(idx){
  ...
  laps.forEach((l,i)=>{
    ...
    lapRows+=`<div style="display:flex;justify-content:space-between;padding:3px 8px;border-radius:4px;background:${isBest?'#22c55e11':'transparent'}">
      <span style="font-size:11.5px;color:var(--text-3)">${i+1}</span>
      <span style="font-size:11.5px;color:${col};font-family:monospace;font-weight:${isBest?'600':'400'}">${_enFmt(l)}${isBest?' ★':''}</span>
    </div>`;
  });
    ...
    <div style="background:#13141a;border:0.5px solid #2a2b2e;border-radius:12px;padding:24px;max-width:400px;width:95%;max-height:85vh;overflow-y:auto">
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">
        <div style="background:#0e0f11;border-radius:8px;padding:10px;text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);text-transform:uppercase;margin-bottom:3px">Duración</div>
          <div style="font-size:14.5px;font-weight:500;color:var(--text-1);font-family:monospace">${_enFmtStint(s.durationMs)}</div>
        </div>
        <div style="background:#0e0f11;border-radius:8px;padding:10px;text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);text-transform:uppercase;margin-bottom:3px">Posición</div>
          <div style="font-size:14.5px;font-weight:500;color:var(--text-1);font-family:monospace">P${posIn}→P${posOut} ${posStr}</div>
        </div>
        <div style="background:#0e0f11;border-radius:8px;padding:10px;text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);text-transform:uppercase;margin-bottom:3px">Vueltas</div>
          <div style="font-size:14.5px;font-weight:500;color:#F5A623;font-family:monospace">${laps.length}</div>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">
        <div style="background:#0e0f11;border-radius:8px;padding:10px;text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);text-transform:uppercase;margin-bottom:3px">Mejor</div>
          <div style="font-size:14.5px;font-weight:500;color:#22c55e;font-family:monospace">${best?_enFmt(best):'—'}</div>
        </div>
        <div style="background:#0e0f11;border-radius:8px;padding:10px;text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);text-transform:uppercase;margin-bottom:3px">M5v</div>
          <div style="font-size:14.5px;font-weight:500;color:var(--text-1);font-family:monospace">${avg5?_enFmt(avg5):'—'}</div>
        </div>
        <div style="background:#0e0f11;border-radius:8px;padding:10px;text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);text-transform:uppercase;margin-bottom:3px">Consist.</div>
          <div style="font-size:14.5px;font-weight:500;color:${cons&&cons<0.5?'#22c55e':cons&&cons<1?'#fbbf24':'#ef4444'};font-family:monospace">${cons?cons.toFixed(2)+'s':'—'}</div>
        </div>
      </div>
      ...
      `:'<div style="font-size:11.5px;color:var(--text-3);text-align:center;padding:12px">Sin datos de vueltas para este stint</div>'}

      ${s.pitTime?`<div style="font-size:11.5px;color:var(--text-3);text-align:center;margin-top:10px;padding-top:8px;border-top:0.5px solid #1a1b22">Parada: ${_enFmtStint(s.pitTime)}</div>`:''}
    </div>`;
  document.body.appendChild(overlay);
}
  ...
  overlay.innerHTML=`
    <div style="background:#1a1b22;border:0.5px solid #2a2b2e;border-radius:12px;padding:24px;max-width:360px;width:90%;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        <div>
          <div style="font-size:11.5px;color:var(--text-2);margin-bottom:4px;font-family:sans-serif">Duración stint</div>
          <div style="display:flex;gap:4px;align-items:center">
            <input type="number" id="en-edit-durmin" value="${durMin}" min="0" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">m</span>
            <input type="number" id="en-edit-dursec" value="${durSec}" min="0" max="59" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">s</span>
          </div>
        </div>
        <div>
          <div style="font-size:11.5px;color:var(--text-2);margin-bottom:4px;font-family:sans-serif">Parada pit</div>
          <div style="display:flex;gap:4px;align-items:center">
            <input type="number" id="en-edit-pitmin" value="${pitMin}" min="0" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">m</span>
            <input type="number" id="en-edit-pitsec" value="${pitSec}" min="0" max="59" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">s</span>
          </div>
        </div>
      </div>
      <div style="display:flex;gap:8px">
        <button onclick="_enApplyStintEdit(${stintIdx})" style="flex:1;padding:8px;border-radius:6px;border:0.5px solid #F5A623;background:#F5A62322;color:#F5A623;font-size:11.5px;cursor:pointer;font-family:sans-serif">Guardar</button>
        <button onclick="_enDismissOverlay()" style="flex:1;padding:8px;border-radius:6px;border:0.5px solid #2a2b2e;background:transparent;color:var(--text-3);font-size:11.5px;cursor:pointer;font-family:sans-serif">Cancelar</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
    ...
    <div style="background:#1a1b22;border:0.5px solid #2a2b2e;border-radius:12px;padding:24px;max-width:360px;width:90%;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        <div>
          <div style="font-size:11.5px;color:var(--text-2);margin-bottom:4px;font-family:sans-serif">Duración stint</div>
          <div style="display:flex;gap:4px;align-items:center">
            <input type="number" id="en-add-durmin" value="0" min="0" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">m</span>
            <input type="number" id="en-add-dursec" value="0" min="0" max="59" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">s</span>
          </div>
        </div>
        <div>
          <div style="font-size:11.5px;color:var(--text-2);margin-bottom:4px;font-family:sans-serif">Parada pit</div>
          <div style="display:flex;gap:4px;align-items:center">
            <input type="number" id="en-add-pitmin" value="0" min="0" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">m</span>
            <input type="number" id="en-add-pitsec" value="0" min="0" max="59" style="background:#0e0f11;border:0.5px solid #2a2b2e;color:var(--text-2);padding:6px;border-radius:4px;font-size:11.5px;width:50px;font-family:monospace;text-align:right">
            <span style="color:var(--text-3);font-size:11.5px">s</span>
          </div>
        </div>
      </div>
      <div style="display:flex;gap:8px">
        <button onclick="_enApplyAddStint()" style="flex:1;padding:8px;border-radius:6px;border:0.5px solid #F5A623;background:#F5A62322;color:#F5A623;font-size:11.5px;cursor:pointer;font-family:sans-serif">Añadir</button>
        <button onclick="_enDismissOverlay()" style="flex:1;padding:8px;border-radius:6px;border:0.5px solid #2a2b2e;background:transparent;color:var(--text-3);font-size:11.5px;cursor:pointer;font-family:sans-serif">Cancelar</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
...
function _enShowLapHistory(dorsal, ev){
  ...
  hist.forEach((t,i)=>{
    ...
    lapsHtml+=`<div style="display:flex;justify-content:space-between;padding:3px 8px;border-radius:4px;${isBest?'background:#22c55e15;':''}${isWorst?'background:#ef444415;':''}">
      <span style="color:var(--text-3);font-size:11.5px;font-family:sans-serif">${hist.length-i}</span>
      <span style="color:${col};font-family:monospace;font-size:13.5px;font-weight:${isBest||isWorst?'600':'400'}">${_enFmt(t)}</span>
    </div>`;
  });
    ...
    <div style="background:#1a1b22;border:0.5px solid #2a2b2e;border-radius:12px;padding:24px;max-width:320px;width:90%;max-height:80vh;display:flex;flex-direction:column;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:14px">
        <div style="text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);font-family:sans-serif">Media</div>
          <div style="font-size:13.5px;color:var(--blue);font-family:monospace">${_enFmt(avg)}</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);font-family:sans-serif">Mejor</div>
          <div style="font-size:13.5px;color:#22c55e;font-family:monospace">${_enFmt(best)}</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:11.5px;color:var(--text-3);font-family:sans-serif">Peor</div>
          <div style="font-size:13.5px;color:#ef4444;font-family:monospace">${_enFmt(worst)}</div>
        </div>
      </div>
      <div style="font-size:11.5px;color:var(--text-3);margin-bottom:6px;font-family:sans-serif">Últimas ${hist.length} vueltas (reciente arriba)</div>
      <div style="overflow-y:auto;flex:1;display:flex;flex-direction:column-reverse;gap:2px">
        ${lapsHtml}
      </div>
      <button onclick="_enDismissOverlay()" style="width:100%;margin-top:12px;padding:8px;border-radius:6px;border:0.5px solid #2a2b2e;background:transparent;color:var(--text-3);font-size:11.5px;cursor:pointer;font-family:sans-serif">Cerrar</button>
    </div>`;
  document.body.appendChild(overlay);
}
    ...
    EnSession.stintHistory.forEach((s,i)=>{
      ...
      html+=`<div class="en-stint-row">
        ...
        <span style="display:flex;gap:2px">
          ...
          <button onclick="_enEditStintPilot(${i})" style="font-size:11.5px;background:none;border:none;color:#F5A623;cursor:pointer;padding:2px" title="Editar stint">✏️</button>
          <button onclick="_enDeleteStint(${i})" style="font-size:11.5px;background:none;border:none;color:var(--text-3);cursor:pointer;padding:2px" title="Borrar stint">🗑</button>
        </span>
      </div>`;
    });
  }
  ...
  if(EnBox.totalStops>0){
    ...
    html+=`<div class="en-team-card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;text-align:center">
        <div>
          <div style="font-size:11.5px;color:var(--text-3);font-family:sans-serif">Hechas</div>
          <div style="font-size:22px;font-weight:600;color:var(--text-2);font-family:monospace">${stopsDone}/${EnBox.totalStops}</div>
        </div>
        <div>
          <div style="font-size:11.5px;color:var(--text-3);font-family:sans-serif">Restantes</div>
          <div style="font-size:22px;font-weight:600;color:var(--blue);font-family:monospace">${stopsRemaining}</div>
        </div>
        <div>
